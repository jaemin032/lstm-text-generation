...
describe('renderTable', () => {
    ...
    it('renders a table', () => {
        ...
        table_1.renderTable({ headers, values }, container);
    ...
    it('requires necessary param', () => {
        const container = document.getElementById('container');
        expect(() => table_1.renderTable({ headers: [] }, container)).toThrow();
        expect(() => table_1.renderTable({ values: [[]] }, container)).toThrow();
        expect(() => table_1.renderTable({}, container)).toThrow();
    });
    it('should not throw on empty table', () => {
        const container = document.getElementById('container');
        const headers = [];
        const values = [];
        expect(() => table_1.renderTable({ headers, values }, container)).not.toThrow();
        expect(document.querySelectorAll('.tf-table').length).toBe(1);
        expect(document.querySelectorAll('.tf-table thead tr').length).toBe(1);
